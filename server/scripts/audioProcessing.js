...
module.exports.audioProcessing = async (req, res) => {
  ...
  if (
    !(
      episodeId &&
      soundcastId &&
      typeof tagging === 'boolean' &&
      typeof overlayDuration === 'number' &&
      typeof setVolume === 'boolean' &&
      typeof trim === 'boolean' &&
      typeof removeSilence === 'number' &&
      ...
      new ffmpeg(filePath).then(
        file => {
          ...
          file.save('-', (err, fileName, stdout, stderr) => {
            ...
            let start = 0,
              end = file.metadata.duration.seconds + 1;
                ...
                file => {
                  ...
                  output.forEach(i => {
                    if (i[0] === 'silence_end') {
                      // not empty block
                      chunksCount++;
                      filterComplex += `[0]atrim=start=${Number(i[1]).toFixed(
                        3
                      )}`;
                    }
                    if (i[0] === 'silence_start') {
                      filterComplex += `:end=${Number(i[1]).toFixed(
                        3
                      )}[a${chunksCount}];`;
                  ...
                  file.addCommand('-q:a', '3');
                  const silenceRemovedPath = `${filePath.slice(
                    0,
                    -4
