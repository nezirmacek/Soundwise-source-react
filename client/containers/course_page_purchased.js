...
import {setCurrentPlaylist, setCurrentCourse, loadUserCourses, openReviewbox} from '../actions/index';

function checkIfUserCourseCompleted (course) {
    let _isUserCourseCompleted = true;
    for (let key in course.sectionProgress) {
        if (course.sectionProgress.hasOwnProperty(key)) {
            if (!course.sectionProgress[key].completed) {
                _isUserCourseCompleted = false;
                break;
            }
        }
    }

    return _isUserCourseCompleted;
}
...
class _Course_Purchased extends Component {
        ...
        firebase.auth().onAuthStateChanged(function(user) {
            if (user) {
                ...
                firebase.database().ref('users/' + userId + '/courses')
                    .on('value', snapshot => {
                        that.props.loadUserCourses(JSON.parse(JSON.stringify(snapshot.val())));
    ...
    componentWillReceiveProps(nextProps) {
        if(
            nextProps.courses[nextProps.match.params.courseId]
            && this.props.courses[this.props.match.params.courseId] == undefined
        ) {
...
function mapDispatchToProps(dispatch) {
    return bindActionCreators({ setCurrentPlaylist, setCurrentCourse, loadUserCourses, openReviewbox }, dispatch);
}
...
export const Course_Purchased = connect(mapStateToProps, mapDispatchToProps)(_Course_Purchased);
