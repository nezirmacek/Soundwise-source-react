...
class _ReviewModal extends Component {
    ...
    handleSubmit() {
        ...
        const pic = (userInfo.pic_url!==undefined && userInfo.pic_url.length > 0) ? userInfo.pic_url : '../images/smiley_face.jpg';
        const reviewer = `${userInfo.firstName} ${userInfo.lastName.slice(0,1)}.`;

        firebase.database().ref('courses/' + course.id)
            .child('reviews').push({date, reviewer, rating, review, pic});
        const userId = firebase.auth().currentUser.uid;
        firebase.database().ref(`users/${userId}/courses/${course.id}/isRated`).set(true)
            .then(
                snapshot => {
