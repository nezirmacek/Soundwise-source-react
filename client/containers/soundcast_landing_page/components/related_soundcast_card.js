                    ...
                    <Card>
                      <Link to={`/soundcasts/${soundcast.id}`}>
                        <div className="float-left width-100 bottom-shadowed" style={_style.bottomShadowed}>
                            <div ref={(image) => this.references.image = image} className="feature-box-image" style={{width: '100%', height: this.references.image.offsetWidth, backgroundColor: 'white'}}>
                                    <CardMedia>
                                        <img alt="" src={soundcast.imageURL}
                                             data-img-size="(W)800px X (H)533px" style={{objectFit: 'cover'}}/>
                                    </CardMedia>
                            </div>
                            <div ref={(content) => this.references.content = content} style={_style.cardContent}>
                                <div style={_style.title}>{soundcast.title.slice(0, 43) + '...'}</div>
                                <div style={_style.courseDescription}>{soundcast.short_description.slice(0, 120) + '...'}</div>
